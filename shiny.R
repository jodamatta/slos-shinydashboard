              ...
              fluidRow(
                box(title = "Upload Data", status = "primary", solidHeader = TRUE, width = 3,
                    ...
                    conditionalPanel(
                      ...
                      div(style = "font-size: 14px; margin-bottom: 5px;",
                          strong("Units within IQR: "), textOutput("units_within_iqr", inline = TRUE)
                      ),
...
server <- function(input, output) {
  ...
  output$funnel_plot <- renderPlot({
    ...
    plot(result_metrics_reactive()$funnel_plot)  # TROCAR TODOS OS slos E metrics POR ISSO
  })
  ...
  output$median_slos <- renderText({
    req(result_metrics_reactive())
    round(result_metrics_reactive()$slos_summary$median_SLOS, 4)
  })
  
  output$units_within_iqr <- renderText({
    req(result_metrics_reactive())
    result_metrics_reactive()$units_within_IQR$n
  })
